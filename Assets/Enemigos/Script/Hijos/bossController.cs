...
public class bossController : Enemy
{
    ...
    [Header("Configuraci�n de salto")] // Para que se muestre un t�tulo encima de las variables
    ...
    private void Update()
    {
        suelo = Physics2D.OverlapCircle(sueloCheck.position, radioSuelo, queEsSuelo); // Comprueba si est� tocando el suelo
        ...
        // Si ha saltado y ya NO est� en el suelo, marcamos que estuvo en el aire
        if (haSaltado && !suelo)
        {
            estuvoEnElAire = true;
        }

        // Si ha saltado, estuvo en el aire, y ahora volvi� al suelo -> reset animaciones
    ...
    {
        Vector2 direccion = (player.transform.position - transform.position).normalized; // Calculamos la direcci�n en la que esta el jugador
